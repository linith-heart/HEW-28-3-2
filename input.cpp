...
void UpdatePad(void)
{
	...
	for (i = 0; i<padCount; i++)
	{
		...
		//* 1ボタン
		if (dijs.rgbButtons[0] & 0x80)		padState[i] |= BUTTON_1;
		//* 2ボタン
		if (dijs.rgbButtons[1] & 0x80)		padState[i] |= BUTTON_2;
		//* 3ボタン
		if (dijs.rgbButtons[2] & 0x80)	padState[i] |= BUTTON_3;
		//* 4ボタン
		if (dijs.rgbButtons[3] & 0x80)	padState[i] |= BUTTON_4;
		//* 5ボタン
		if (dijs.rgbButtons[4] & 0x80)	padState[i] |= BUTTON_5;
		//* 6ボタン
		if (dijs.rgbButtons[5] & 0x80)	padState[i] |= BUTTON_6;
		//* 7ボタン
		if (dijs.rgbButtons[6] & 0x80)	padState[i] |= BUTTON_7;
		//* 8ボタン
		if (dijs.rgbButtons[7] & 0x80)		padState[i] |= BUTTON_8;
		//* 9ボタン
		if (dijs.rgbButtons[8] & 0x80)		padState[i] |= BUTTON_9;
		//* 10ボタン
		if (dijs.rgbButtons[9] & 0x80)	padState[i] |= BUTTON_10;
		//* 11ボタン
		if (dijs.rgbButtons[10] & 0x80)	padState[i] |= BUTTON_11;
		//* 12ボタン
		if (dijs.rgbButtons[11] & 0x80)	padState[i] |= BUTTON_12;
		//* z-axis (forward)
